...
const FEATURE_MATCAP_EGRN_UPLOAD =
  process.env.REACT_APP_FEATURE_MATCAP_EGRN_UPLOAD !== "false";
...
const MaternityObjectRightsSection = ({
  ...
  const updateSection = (section, key, value) => {
    setFormData((prev) => {
      ...
      if (section === "agreement" && key === "date") {
        next.participantsStep = {
  ...
  const selectedHouseStub = [
    "house_with_land",
    "house_with_land_share",
  ].includes(formData.acquisition.type);
  const isShareLike =
    formData.acquisition.type === "apartment_share" ||
    formData.acquisition.type === "communal_room_share";
  ...
  const fallbackOwnerBlocks = [
    {
      ...
      fullName: (formData.rights.owners || [])
      ...
      shareDisplayText:
        formData.rights.existingShareDisplayText ||
        formData.rights.existingShare ||
        "",
  ...
  const syncLegacyRightsFromOwnerBlocks = (blocks) => {
    ...
    return {
      ...
      existingShareDisplayText:
        firstBlock.shareDisplayText || formData.rights.existingShareDisplayText,
      registrationNumber:
        firstBlock.registrationNumber ||
        firstGroup.regNumber ||
        formData.rights.registrationNumber,
      registrationDate:
        firstBlock.registrationDate ||
        firstGroup.regDate ||
  ...
  return (
    <div className="space-y-6">
      ...
      {FEATURE_MATCAP_EGRN_UPLOAD &&
          ...
          <section className={cardClass}>
            ...
            <label className="block border-2 border-dashed border-gray-300 rounded-xl p-8 text-center hover:border-blue-400 transition-colors cursor-pointer">
              ...
              <span className="block text-gray-900 font-medium">
                Выберите PDF или ZIP-файл
              </span>
              <span className="block text-sm text-gray-500 mt-2">
                Данные не будут применены автоматически.
              </span>
            </label>
            {formData.ui.parseStatus === "parsing" && (
              <p className="mt-4 text-blue-600">Разбираем выписку…</p>
            )}
            {formData.ui.parseStatus === "error" && (
              <div className="mt-4 rounded-lg bg-red-50 text-red-700 p-4">
                {formData.ui.parseWarnings.join(" ") ||
        ...
        {parsedEgrn &&
        formData.ui.parseStatus === "parsed" &&
        !formData.egrn.applied && (
          <section className={cardClass}>
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              Найдены данные из ЕГРН
            </h3>
            <dl className="mb-6">
              ...
              <SummaryRow
                label="Вид объекта"
                value={parsedEgrn.object.objectKindFromEgrn}
              />
              <SummaryRow
                label="Вид жилого помещения"
                value={parsedEgrn.object.residentialKind}
              />
              <SummaryRow
                label="Правообладатели"
                value={parsedEgrn.rights.owners
                  .map((owner) => owner.fullName)
                  .filter(Boolean)
                  .join(", ")}
              />
              ...
              <SummaryRow
                label="Доля"
                value={parsedEgrn.rights.existingShare}
              />
              <SummaryRow
                label="Регистрация"
                value={[
                  parsedEgrn.rights.registrationNumber,
                  parsedEgrn.rights.registrationDate,
                ]
                  .filter(Boolean)
                  .join(" от ")}
              />
              <SummaryRow
                ...
                value={parsedEgrn.rights.basisDocuments
                  .flatMap((group) =>
                    group.basisDocuments.map(
                      (doc) =>
                        `${doc.title}${doc.docDate ? `, от ${doc.docDate}` : ""}`,
                    ),
                  )
                  .join("\n")}
              />
              <SummaryRow
                label="Обременения"
                value={getEncumbranceDisplayValue(parsedEgrn.encumbrance)}
              />
              {parsedEgrn.encumbrance.type === "mortgage" && (
                <>
                  <SummaryRow
                    ...
                    value={[
                      parsedEgrn.encumbrance.registrationNumber,
                      parsedEgrn.encumbrance.registrationDate,
                    ]
                      .filter(Boolean)
                      .join(" от ")}
                  />
                  <SummaryRow
                    ...
                    value={
                      parsedEgrn.encumbrance.mortgagee ||
                      parsedEgrn.encumbrance.beneficiary
                    }
                  />
                  <SummaryRow
              ...
              <SummaryRow
                label="Статус записи"
                value={parsedEgrn.object.recordStatus}
              />
              <SummaryRow
                label="Дата актуальности"
                value={parsedEgrn.object.egrnActualDate}
              />
            </dl>
            ...
            {parsedEgrn.recipientOwnerMatch.matched && (
              <div className="mb-4 rounded-lg bg-green-50 text-green-800 p-4">
                {parsedEgrn.rights.ownershipType === "Собственность"
                  ...
                  : parsedEgrn.rights.ownershipType ===
            ...
            {!parsedEgrn.recipientOwnerMatch.matched &&
              parsedEgrn.recipientOwnerMatch.recipientName && (
                <div className="mb-4 rounded-lg bg-yellow-50 text-yellow-800 p-4">
                  Получатель выписки не был однозначно найден среди
                  правообладателей. Проверьте данные вручную.
                </div>
              )}
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <button
                type="button"
                onClick={onRejectEgrn}
                className={`${btnBase} bg-gray-100 text-gray-900 hover:bg-gray-200`}
              >
        ...
        <>
          <section className={cardClass}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Field
                ...
                value={formData.agreement.place}
                onChange={(value) => updateSection("agreement", "place", value)}
                placeholder="г. Санкт-Петербург"
              />
              <Field
                label="Дата соглашения"
                value={formData.agreement.date}
                onChange={(value) => updateSection("agreement", "date", value)}
          ...
          <section className={cardClass}>
            ...
            {formData.acquisition.reason && (
              <div
                className={`mb-4 rounded-lg p-4 ${formData.acquisition.confidence === "manual_required" ? "bg-yellow-50 text-yellow-800" : "bg-blue-50 text-blue-800"}`}
              >
                {formData.acquisition.reason}
              </div>
            )}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <TextAreaField
                ...
                onChange={(value) => updateSection("object", "address", value)}
              />
              <Field
                label="Кадастровый номер"
                value={formData.object.cadastralNumber}
                onChange={(value) =>
                  updateSection("object", "cadastralNumber", value)
                }
              />
              <Field
                label="Площадь, кв. м"
                value={formData.object.area}
                onChange={(value) => updateSection("object", "area", value)}
              />
              <Field
                label="Этаж"
                value={formData.object.floor}
                onChange={(value) => updateSection("object", "floor", value)}
              />
              <Field
                label="Вид объекта из ЕГРН"
                value={formData.object.objectKindFromEgrn}
                onChange={(value) =>
                  updateSection("object", "objectKindFromEgrn", value)
                }
              />
              <Field
                label="Вид жилого помещения"
                value={formData.object.residentialKind}
                onChange={(value) =>
                  updateSection("object", "residentialKind", value)
                }
              />
              <label className="block md:col-span-2">
                <span className={labelClass}>
                  Что приобреталось с использованием материнского капитала?
                </span>
                <select
                  value={formData.acquisition.type || ""}
                  onChange={(event) =>
                    updateAcquisitionType(event.target.value)
                  }
                  className={inputClass}
                >
              ...
              {isShareLike && (
                <>
                  <Field
                    label="Доля объекта, приобретённая с использованием материнского капитала"
                    value={formData.object.purchasedShare}
                    onChange={(value) =>
                      updateSection("object", "purchasedShare", value)
                    }
                  />
                  <Field
                    label="Номер комнаты"
                    value={formData.object.roomNumber}
                    onChange={(value) =>
                      updateSection("object", "roomNumber", value)
                    }
                  />
                  <Field
                    label="Фактическая площадь комнаты"
                    value={formData.object.roomArea}
                    onChange={(value) =>
                      updateSection("object", "roomArea", value)
                    }
                  />
                  <Field
                    label="Жилая площадь квартиры"
                    value={formData.object.livingArea}
                    onChange={(value) =>
                      updateSection("object", "livingArea", value)
                    }
                  />
                  <Field
                    label="Юридическая доля"
                    value={formData.object.legalShare}
                    onChange={(value) =>
                      updateSection("object", "legalShare", value)
                    }
                  />
                  <p className="md:col-span-2 text-sm text-gray-500">
                    ЕГРН показывает текущее зарегистрированное право. Если с
                    использованием материнского капитала приобреталась не вся
                    квартира, укажите вручную, какая именно доля приобреталась.
                  </p>
                </>
              )}
            </div>
