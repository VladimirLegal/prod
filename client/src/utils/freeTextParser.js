...
function extractAllDates(raw) {
  ...
  const re =
    /([0-3]?\d[./-][01]?\d[./-](\d{4}|\d{2}))|([0-3]?\d\s+[А-Яа-яё]+\s+\d{4}(?:\s+г(?:ода)?\.?)?(?:\s+рождения)?)/gi;
  const out = [];
...
function stripDatesFromPlace(s) {
  ...
  if (mWord)
    cutIdx = cutIdx === -1 ? mWord.index : Math.min(cutIdx, mWord.index);
...
export function normalizeSnils(s) {
  if (!s) return "";
...
function cutAroundFirstPassport(text) {
  const re =
    /(?:пас+порт|паспорт)(?:\s+гражданина\s+российской\s+федерации)?|Серия\s*и\s*номер|серия|№/iu;
...
const BAD_TOKENS =
  /(гор\.?|г\.|ул\.?|просп\.?|пр-т|район|р-?н|обл\.?|край|края|лит\.?|дом|д\.|корп\.?|кв\.?|снилс|именуем)/i;
...
export function parseFreeTextPerson(raw) {
  ...
  const passportPivotMatch =
    /(?:пас+порт|Серия\s*и\s*номер|серия|номер|выдан(?:ный|а|о|ы)?)/iu.exec(
      text,
    );
  ...
  if (!passport) {
    const m = text.match(
      /серия\s*[:\-]?\s*([0-9]{2}\s?[0-9]{2}).*?номер\s*[:\-]?\s*([0-9][0-9\s]{5,})/i,
    );
  ...
  if (passportIssued) {
    ...
    const lead = passportIssued.match(
      /^([0-3]?\d[./-][01]?\d[./-]\d{2,4}|[0-3]?\d\s+[А-Яа-яё]+\s+\d{4})\s*(?:г(?:ода)?\.?)?\s*(.*)$/i,
    );
    if (lead) {
  ...
  const snils = normalizeSnils(
    grab(text, [
      /СНИЛС\s*[:\-]?\s*([0-9]{3}[-\s]?[0-9]{3}[-\s]?[0-9]{3}[-\s]?[0-9]{2})/i,
      /\b([0-9]{3}[-\s]?[0-9]{3}[-\s]?[0-9]{3}[-\s]?[0-9]{2})\b/i,
    ]),
  );
  ...
  if (!birthPlace) {
    ...
    cut =
      cut.split(
        /СНИЛС|пас+порт|Серия\s*и\s*номер|серия|номер|выдан|код\s*подразделения|зарегистр|телефон|email|e-?mail|именуем|пол/iu,
      )[0] || "";
    cut = cut.replace(/^\s*[,;]\s*/, "");
  ...
  if (!birthDate || !issueDate) {
    ...
    if (parsed.length) {
      ...
      if (!birthDate && birthCands.length) {
        ...
        birthDate = birthCands.reduce((a, b) =>
          _dateNum(b.s) < _dateNum(a.s) ? b : a,
        ).s;
      }
      ...
      if (!issueDate && issueCands.length) {
        ...
        issueDate = issueCands.reduce((a, b) =>
          _dateNum(b.s) > _dateNum(a.s) ? b : a,
        ).s;
      }
...
export function parseBirthCertificateText(raw) {
  const text = String(raw || "")
    .replace(/\r/g, "")
    .replace(/\u00A0/g, " ")
    .trim();
  ...
  let fullName = grab(text, [
    /^\s*([А-ЯЁ][а-яё-]+(?:\s+[А-ЯЁ][а-яё-]+){1,3})\s*,/m,
    /(?:несовершеннолетн(?:его|ей)|реб[её]н(?:ка|ок))\s+([А-ЯЁ][а-яё-]+(?:\s+[А-ЯЁ][а-яё-]+){1,3})/i,
    /ФИО\s*(?:реб[её]нка)?\s*[:\-]?\s*([^\n,]+)/i,
  ]);
  if (!fullName) {
    const firstLine = text.split(/\n/)[0] || "";
    const beforeComma = firstLine.split(",")[0];
    const words = beforeComma.match(/[А-ЯЁ][а-яё-]+/g) || [];
    if (words.length >= 2) fullName = words.slice(0, 4).join(" ");
  }
  ...
  let series = "";
  let number = "";
  const docMatch = flat.match(
    /свидетельство\s+о\s+рождении\s+([A-ZА-ЯЁIХVXLCDM0-9-]+)\s*№?\s*([0-9]{3,})/i,
  );
  if (docMatch) {
    series = docMatch[1].trim();
  ...
  const issueMatch = flat.match(
    /выдано[\s\S]*?([0-3]?\d[./-][01]?\d[./-]\d{2,4})\s*(?:г\.|года)?/i,
  );
  if (issueMatch) issueDate = normalizeDateSmart(issueMatch[1]);
  ...
  if (actMatch) {
    ...
    actRecordDate = normalizeDateSmart(actMatch[2]);
  }
